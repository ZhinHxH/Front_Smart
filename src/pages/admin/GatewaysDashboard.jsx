...
const GatewayDashboard = ({ sidebar }) => {
    ...
    useEffect(() => {
        ...
        // Crear un objeto URLSearchParams para agregar los parámetros de consulta
        const params = new URLSearchParams();
        params.append('start_date', fecha_gte);
        params.append('end_date', fecha_lte);
    
        selectedKeys ? selectedKeys.forEach(center => {
            params.append('service_centers', center);
        }) : null;
    
        const endPoint = isExclusive ? "exclusivos-por-gateway" : "no-exclusivos-por-gateway";
    
        // Crear un controlador de aborto para la solicitud
        const controller = new AbortController();
        const signal = controller.signal;
    
        fetch(`http://localhost:8000/api/v1/${endPoint}/?${params.toString()}`, { signal })
            .then(response => response.json())
            .then(data => {
                setIsLoading(true);
                console.log("EndPoint: ", data);
    
                filteredGateway ? setMetersLength(data.find(item => item.gateway_id === filteredGateway).total_lecturas) : setMetersLength(data[0].total_lecturas);
                ...
                const groupedData = data.reduce((acc, item) => {
                    ...
                    if (!acc[service_center]) {
                        acc[service_center] = {
                            service_center,
                            total_lecturas: 0, // Sumar lecturas por cada service_center
                            gateways: []
                        };
                    }
                    acc[service_center].total_lecturas += total_lecturas;
                    acc[service_center].gateways.push(item.gateway_id); // Guardar los gateway_id relacionados
                    return acc;
                }, {});
                ...
                const colorMap = labels.reduce((acc, center, index) => {
                    acc[center] = `hsl(${(index * 360) / labels.length}, 70%, 50%)`; // Asignar un color único por service_center
                    return acc;
                }, {});
        ...
        return () => {
            // Abortar la solicitud anterior si el efecto se ejecuta nuevamente
            controller.abort();
        };
